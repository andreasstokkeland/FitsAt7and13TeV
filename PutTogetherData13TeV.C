...
void Sphere(const Char_t* fileName="HEPData-ins1784041-v1-Table_1.root", const Char_t* fileMapp="Table 1", const Char_t* filePart="Hist1D_y1", const Char_t* fileParty1="Hist1D_y1_e1", const Char_t* fileParty2="Hist1D_y1_e2", const Char_t* fileParty3="Hist1D_y1_e3") {
    ...
    TH1D* h = (TH1D*)file2->Get(filePart);
    
    
    TH1D* hstat = (TH1D*)file2->Get(fileParty1);

    
    TH1D* hsystfull = (TH1D*)file2->Get(fileParty2);
    ...
    TCanvas * c = new TCanvas( Form("%s%s.root", fileName, filePart ) );
...
*/
    ...
    h->SetName(Form("%s%s.root", fileName, filePart ));
    
    h->SaveAs(Form("%s/%s%s.root", localDirName.Data(), fileName, filePart ));
    
    c->SaveAs(Form("%s/%s%s.root.C", localDirName.Data(), fileName, filePart ));
    
    
    c->SaveAs(Form("%s/%s%s.eps", localDirName.Data(), fileName, filePart ));
