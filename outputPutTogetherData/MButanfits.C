...
//    HistosPion7[0]->GetYaxis()->SetTitleSize(SIZE);
    ...
    for (int i = 0; i < 9 ; i++){
        ...
        HistosMBKaon7[i]->SetTitle( "Multiplicity classes over MB" ); //Levi fits for each multiplicity divided by Levi fit for min bias at #sqrt{s} = 13 TeV for K#pm
...
//    lName13->Draw();
    ...
    for (int i = 0; i < 9 ; i++){
        ...
        HistosMBKaon13[i]->SetTitle( "" ); //ratios of Levi fits for multiplicity classes and MB
